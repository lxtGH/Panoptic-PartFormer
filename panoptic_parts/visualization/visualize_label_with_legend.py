...
def visualize_from_paths(datasetspec_path, label_path):
    ...
    uids = encode_ids(
        *decode_uids(uids, experimental_dataset_spec=spec, experimental_correct_range=True))
    ...
    for uid in uids_unique:
        h = plt.Rectangle((0, 0), 1, 1, fc=list(
            map(lambda x: x/255, uid2color_dct[uid])))
        handles.append(h)
    ...
    ax1.set_title(
        'labels colored on semantic, instance, and part levels', fontsize='small')
