...
class PQStat():
    ...
    def pq_average(self, cat_definition):
        ...
        return [{'PartPQ': pq / n, 'PartSQ': sq / n, 'PartRQ': rq / n, 'n': n},
                ...
                {'PartPQ_noparts': pq_np / n_np, 'PartSQ_noparts': sq_np / n_np, 'PartRQ_noparts': rq_np / n_np,
                 'n_np': n_np}], per_class_results
            ...
            for j in range(num_ins_pred):
                ...
                if mask_iou > 0.5:
                    ...
                    if len(cat_definition['cat_def'][cat_id]['parts_cls']) > 1:
                        ...
                        cm = confusion_matrix(temp_gt_parts[msk_evaluated].reshape(-1),
                                              temp_pred_parts[msk_evaluated].reshape(-1))
...
def evaluate_PartPQ_multicore(spec,
    ...
    # p = evaluate_single_core(0, fn_splits[0], pred_reader_fn, spec)
    # results = p.pq_average(cat_definition)
    #
