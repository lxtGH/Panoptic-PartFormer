...
PARULA99_INT = [
    ...
    (81, 203, 126), (89, 204, 119), (97, 204,
                                     112), (106, 204, 105), (115, 204, 98),
...
PARULA99_FLOAT = list(
    map(lambda t: tuple(map(lambda c: c/255, t)), PARULA99_INT))
# parula_cm(x), x can be float in [0.0, 1.0] or int in [0, 99) to return a color
PARULA99_CM = matplotlib.colors.LinearSegmentedColormap.from_list(
    'parula99', PARULA99_FLOAT, 99)
...
def _generate_shades(center_color, deltas, num_of_shades):
    ...
    if num_of_shades <= 0:
        raise ValueError(
            f"num_of_shades must be a positive integer (was {num_of_shades}).")
    if num_of_shades == 1:
        return [center_color]
    # TODO: enable d=0
...
def _validate_uid2color_args(uids, sid2color, experimental_deltas, experimental_alpha):
    ...
    if not isinstance(uids, (list, np.ndarray)):
        raise ValueError(
            f"Provide a list or np.ndarray of uids. Given {type(uids)}.")
    if isinstance(uids, np.ndarray):
        uids = list(map(int, np.unique(uids)))
    if not all(map(isinstance, uids, [int]*len(uids))):
        raise ValueError(
            f"Provide a list of Python ints as uids. Given {uids}.")
    if not all(map(lambda uid: 0 <= uid <= 99_999_99, uids)):
        raise ValueError(
            f'There are uids that are not in the correct range. Given {uids}.')
    # sid2color checks
    ...
    sids_unique_from_uids = set(
        map(operator.itemgetter(0), map(decode_uids, uids)))
    if not sids_unique_from_uids.issubset(sid2color):
        raise ValueError(
            f"Not all sids in uids have a matching color in sid2color.")
    # experimental_deltas checks
    if not isinstance(experimental_deltas, tuple):
        raise ValueError(
            f"experimental_deltas must be a tuple. Given {type(experimental_deltas)}.")
...
def uid2color(uids,
    ...
    if VALIDATE_ARGS:
        _validate_uid2color_args(
            uids, sid2color, experimental_deltas, experimental_alpha)
...
def experimental_colorize_label(label, *,
    ...
    palette = _sparse_ids_mapping_to_dense_ids_mapping(
        uid2color_dict, (0, 0, 0), dtype=np.uint8)
    ...
    if emphasize_instance_boundaries:
        ...
        sids_iids_concat = np.where(
            sids_iids > 100, sids_iids, np.zeros_like(sids_iids))
        ...
        if edge_option == 'sobel':
            boundaries_image = np.uint8(
                edges)[..., np.newaxis] * np.uint8([[[255, 255, 255]]])
        elif edge_option == 'erosion':
            boundaries_image = np.uint8(
                boundaries)[..., np.newaxis] * np.uint8([[[255, 255, 255]]])
