...
def evaluate(eval_spec_path, basepath_gt, basepath_pred, images_json, save_dir=None):
    ...
    print()
    res = (*map(lambda d: ', '.join(map(lambda t: f'{t[0]}: {t[1]:.3f}', d.items())),
                results[0]),
           *map(lambda t: f'{t[0]:15} ' + ', '.join(map(lambda t: f'{t[0]}: {t[1]:.3f}', t[1].items())),
    ...
    if save_dir is not None:
        ...
        #         exit()
        with open(filepath_output, 'a+') as fp:
            fp.writelines('\n\n')
...
if __name__ == '__main__':
    ...
    args.eval_spec_path = '/home/xsl/project/PanopticPartSeg/panoptic_parts/specs/eval_specs/ppq_cpp_19_23_cvpr21_default_evalspec.yaml'
    args.basepath_gt = '/data/cityscapes/gtFinePanopticParts/val'
    args.basepath_pred = '/data/cityscapes/part_pred_res/'
    args.images_json = '/data/cityscapes/annotations/val_images.json'
    args.save_dir = '/home/xsl/'
